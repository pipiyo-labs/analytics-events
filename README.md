# Pipiyoswap Labs Analytics Events
[![npm](https://img.shields.io/npm/v/@pipiyo/analytics-events)](https://www.npmjs.com/package/@pipiyo/analytics-events)

The `@pipiyo/analytics-events` package is a [npm package](https://www.npmjs.com/package/@pipiyo/analytics-events) of constants used as the inputs into our [@uniswap/analytics](https://www.npmjs.com/package/@pipiyo/analytics) package ([Github Link](https://github.com/pipiyo-labs/analytics)). It is used in repos such as [interface](https://github.com/pipiyo-labs/interface) and [docs](https://github.com/pipiyo-labs/docs).
...
```bash
yarn add @pipiyo/analytics-events
```

```bash
npm i --save @pipiyo/analytics-events
```
...
The [analytics](https://github.com/pipiyo-labs/analytics) library implements the [Trace framework](https://slack.engineering/creating-a-react-analytics-logging-library/) to enable easy default properties and basic hierarchy of context. The following context properties are available, in order of specificity. This can be used directly for ease or logged manually for custom trigger events.
...
This will generate a `pipiyoswap-analytics-events-dev.tgz` including your changes and a `0.0.1` package version number.
...
# yarn
yarn cache clean
yarn add file:pipiyoswap-analytics-dev.tgz

# npm
npm install pipiyoswap-analytics-dev.tgz
```
...
Pipiyo Labs encourages integrators to evaluate their own regulatory obligations when integrating this package into their products, including, but not limited to, those related to economic or trade sanctions compliance.